...
class InnerP(UserExpression):
    ...
    def eval(self, value, x):
        if self.t >= 0.05:
            value[0] = 10000*(self.t-0.05)**2
        else:
            value[0] = 0.0 # lower slope

    def value_shape(self):
        return ()


def create_bcs(DVP, v_deg,p_deg, Um, H, mesh,boundaries, extrapolation_sub_type,theta,ds,psi,mu_f,rho_f,d_,v_,dx_f,F_fluid_nonlinear,F_fluid_linear, **namespace):
    ...
    # Pressure Conditions
    p_out_bc_val = InnerP(t=0.0, degree=p_deg)
    p_out = DirichletBC(DVP.sub(2), p_out_bc_val, boundaries, 4) # Dirichlet BC, causes divergence

    # Modify variational form
    # dso = ds(4, domain=mesh, subdomain_data=boundaries) # Outlet surface
    # n_ = FacetNormal(mesh)
    
    # Add pressure as flux at boundary
    # F_fluid_nonlinear += p_out_bc_val * inner(n_, psi)*dso
    ...
    return dict(bcs=bcs, inlet=inlet, p_out_bc_val=p_out_bc_val)
