...
def initiate(F_solid_linear, theta, dx_mms, dy_mms, dt, lambda_s, mu_s,
             d_deg, eps, mesh, psi, dvp_, **namespace):
    ...
    for th, t_n in [(theta, "t_e_n"), ((1 - theta), "t_e_n_1")]:
        ...
        f_tmp = diff(diff(d_vec, t_n), t_n) + div(Piola1(d_vec, lambda_s, mu_s))

        F_solid_linear -= th * inner(f_tmp, psi)*dx_f[solid_region]

    # Set manufactured solution as initial condition for n-1 (t = 0)
    assign(dvp_["n-1"].sub(0).sub(0), project(dx_e, dvp_["n"].sub(1).sub(0).function_space().collapse()))
    assign(dvp_["n-1"].sub(0).sub(1), project(dy_e, dvp_["n"].sub(1).sub(1).function_space().collapse()))

    return dict(dx_e=dx_e, dy_e=dy_e, t_e_n=t_e_n, t_e_n_1=t_e_n_1,
