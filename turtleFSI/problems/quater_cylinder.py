...
def set_problem_parameters(default_variables, **namespace):
    ...
    default_variables.update(dict(
        ...
        recompute_tstep=110,
        robin_bc=True,
        k_s=[1E5],  # Stiffness for Robin boundary condition
        c_s=[0],  # Damping for Robin boundary condition
        ds_s_id=[3],  # Subdomain id for Robin boundary condition

        # Problem specific
        dx_s_id=1,     # Id of the solid domain
        folder="quater_cylinder",          # Folder to store the results
...
def get_mesh_domain_and_boundaries(dx_s_id, **namespace):
    ...
    mesh_path = "/Users/keiyamamoto/Documents/MyMesh/cylinder/quater_cylinder/mesh.xdmf"
    
    mesh = Mesh()
    with XDMFFile(mesh_path) as infile:
        infile.read(mesh)

    mf_path = "/Users/keiyamamoto/Documents/MyMesh/cylinder/quater_cylinder/mf.xdmf"
...
def create_bcs(F_solid_linear, DVP, boundaries, P_final, t_start_p, t_end_p, mesh, psi, **namespace):
    ...
    bcs = [bc_bottom, bc_left, bc_inlet, bc_outlet]
